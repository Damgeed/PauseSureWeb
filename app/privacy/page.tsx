...
export default function Privacy() {
  return <LegalShell title="Privacy should feel understandable." intro="This Privacy Notice explains how PauseSure handles personal data across the website, browser checker, and iPhone app features you choose to use.">
    ...
    <div className="legal-notice">The public checker sends the text, phone number, or destination you submit to PauseSure’s analysis service. Submitted content is processed for the requested check, is not written to application data or request logs, and is not used for behavioral advertising. Valid web addresses may be sanitized and checked through Google Web Risk without opening the destination. PauseSure does not sell personal data.</div>
    ...
    <p>The public checker offers an optional, off-by-default control to share aggregate product events. If you enable it, PauseSure stores counts such as the type of check completed, broad result category, and next-action category. These counts measure feature use and flow reliability; they cannot identify false negatives, provide labeled examples, calibrate detection accuracy, or train a classifier. The analytics event does not contain the entry you checked, URLs, phone numbers, images, contact information, account or session identifiers, device identifiers, IP addresses, or free-form text. Daily aggregate rows are retained for up to 180 days. You can turn this choice off from the checker at any time.</p>
    <h2>URL threat intelligence</h2>
    <p>For URL threat checks, PauseSure sends each bounded web address to the PauseSure reputation gateway without the surrounding message or image. Credentials embedded in the address and fragments are removed; the remaining path and query are kept because changing them could check a different destination. The gateway keeps the Google credential on the server and does not write the submitted address to application data or request logs. Google receives the resulting address to compare it with Web Risk threat lists. PauseSure does not open the submitted site, so the lookup does not determine whether it currently responds, has been deactivated, or later returns. Threat matches and short failure backoff may be cached under a SHA-256 key; Lookup no-match responses are not cached. A no-known-match response is point-in-time evidence, not proof that the destination is safe.</p>
    ...
    <p>In the iPhone app, selected evidence may be converted into text before you request analysis. When you run a check, the content you deliberately submit is sent to PauseSure’s analysis service. Other features you deliberately connect may use PauseSure services for account sessions, notification delivery, encrypted Trusted Circle routing, encrypted backup, content-free analytics, and configured reputation lookups. These paths may process submitted check content, account identifiers, hashed session records, encrypted notification tokens, ciphertext, routing metadata, and narrowly scoped security events. A connected feature is not permission to collect unrelated device content.</p>
    <h2>Connected-content boundaries</h2>
    <p>Trusted Circle request content and cloud backup content are encrypted before upload. The service still needs limited routing metadata, such as account identifiers, sender and recipient relationships, request status, timestamps, expiry, and encrypted payload size. A provider outage, missing feed, or failed lookup produces Couldn’t verify—not a reassuring result.</p>
    ...
    <p>PauseSure uses service providers for website hosting, security, email delivery, and URL threat intelligence. Google Cloud processes bounded link indicators for Web Risk lookups under its applicable terms and safeguards. Links to banks, government agencies, reporting services, and other websites are governed by those organizations&apos; notices.</p>
